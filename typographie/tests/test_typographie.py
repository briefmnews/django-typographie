...
class TestTypographieSpaces:
    def test_div(self):
        ...
        assert typographie(text) == "<div>un test\xa0: test</div>"

    def test_column(self):
        text = "un test : test"
        assert typographie(text) == "un test\xa0: test"
    ...
    def test_with_quote(self):
        ...
        assert typographie(text) == "<strong>\xab\xa0test\xa0\xbb</strong>."

    def test_nonbreakable_after_digit(self):
        text = "10 personnes"
        assert typographie(text) == "10\xa0personnes"

    def test_nonbreakable_between_digits(self):
        text = "10 000"
        assert typographie(text) == "10\xa0000"

    def test_nonbreakable_between_digit_percent(self):
        text = "10%"
        assert typographie(text) == "10\xa0%"
        text = "10 %"
        assert typographie(text) == "10\xa0%"
